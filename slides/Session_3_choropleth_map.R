...
pardefault <- par(no.readonly=T) # save default base R plotting parameters.  To restore defaults, call par(pardefault)
...
esri
ggplot(ct, aes(x=date, y=state, alpha=stayhome_active, fill=stayhome_active)) + 
  geom_raster()
...
elec_chplt + 
  scale_fill_gradient2(low="deepskyblue", mid="purple", high="red", na.value="black")
...
# same code; let's look at just CA
ggplot() +
  geom_polygon(data=subset(choro_df, state=="California"), 
               aes(x=long, y=lat, group=group, fill=cases), 
               colour="white") +
  labs(fill="cumulative cases", 
       title=paste0("cumulative case count by county, ", maxdate),
